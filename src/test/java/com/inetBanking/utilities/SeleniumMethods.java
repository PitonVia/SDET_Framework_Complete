...
//			JavascriptExecutor js = (JavascriptExecutor) driver;
			jse.executeScript("scroll(" + intX + "," + intY + ")");
...
//		JavascriptExecutor js = (JavascriptExecutor) driver;
		try {
			...
			if (elementPresent == true) {
				jse.executeScript("arguments[0].click();", elementPresent);
...
//			Assert.fail("WebElement is NOT visible, Exception: " + e.getMessage());
	...
	public void waitForAlert() throws InterruptedException {
		...
		int i=0;
	   // total wait up to 5000: (20 * 0.250)
	    while(i++ < 20) {
	    	try {
...
/*
	...
	// Generate a random alphabetic string of any length:
	public String getRandomAlphabeticString(int length) {		
		
		return RandomStringUtils.randomAlphabetic(length);
	}
	
	// Generate a random numeric string of any length:
	public String getRandomNumericString(int length) {	
		
		return RandomStringUtils.randomNumeric(length);
	}
	
	// Generate a random integers of any length:
	public int getRandomIntegers(int length){
